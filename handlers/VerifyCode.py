...
class SMSCodeHandler(RequestHandler):
    """手机验证码"""
    def get(self):
        ...
        if image_code.lower() != real_image_code.lower():
            self.application.redis.delete("ImageCode" + image_code_id)
            return self.write({"errno":3, "errmsg":"图片验证码错误"})
        db = DataBase()
        res = db.query_one("select count(*) counts from ih_user_profile where up_mobile=%(mobile)s", mobile=mobile)
        if 0 != res['counts']:
            return self.write({"errno":4, "errmsg":"该手机号已存在"})    
        sms_code = '%04d' % random.randint(0, 10000) 
        self.application.redis.setex("SMSCode" + mobile, constants.SMS_CODE_VALIDITY, sms_code)
        try:
